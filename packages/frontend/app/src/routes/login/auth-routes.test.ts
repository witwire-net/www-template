...
describe('[AUTH-FE-S001] 公開面の低強調 handoff から /app/login へ到達する', () => {
  it('login handoff link は低強調で /app/login を指す', () => {
    /* Route-level test: 公開面 footer にある login リンクは /app/login を href とし、
...
describe('[AUTH-FE-S002] ログイン画面はパスキー専用でサインインを提供する', () => {
  it('/app/login route は passkey-only であり password / invite UI を含まない contract を持つ', () => {
    /* /app/login route の contract 検証:
       - passkey sign-in action を表示する
        - recovery link (/app/login/recovery) を提供する
...
describe('[AUTH-FE-S006] セッション失効時は再認証画面へリダイレクトする', () => {
  it('expired session は /app/session-expired へ分岐する', () => {
    const state = createAuthSessionInitialState();
    const intent = applyExpiredSession(state, 'no-store');

    expect(intent).toBe('/app/session-expired');
    expect(state.phase).toBe('session-expired');
    expect(state.session).toBeNull();
    expect(state.routeIntent).toBe('/app/session-expired');
  ...
  it('expired session と missing session を区別する', () => {
    ...
    expect(expiredIntent).toBe('/app/session-expired');
    expect(missingIntent).toBe('/app/login');
...
describe('[AUTH-FE-S007] logout は利用者を非認証 route へ戻す', () => {
  it('clearAuthSession は anonymous phase に戻し /app/login intent を返す', () => {
    const state = createAuthSessionInitialState();
    ...
    expect(intent).toBe('/app/login');
    expect(state.phase).toBe('anonymous');
    expect(state.session).toBeNull();
    expect(state.lastFailure).toBeNull();
  });

  it('/app/logout route metadata は no-store を宣言する', () => {
    expect(LOGOUT_CACHE_POLICY).toBe('no-store');
  });
});

describe('[AUTH-FE-S008] session を持たない /app/* 到達は通常の未認証導線に留まる', () => {
  it('missing session は /app/login に留まり session-expired にならない', () => {
    const state = createAuthSessionInitialState();
    const intent = applyMissingSession(state, 'no-store');

    expect(intent).toBe('/app/login');
    expect(state.routeIntent).toBe('/app/login');
...
describe('[AUTH-FE-S009] auth routes は no-store surface として配信される', () => {
  it('/app/login route layout は no-store cache policy を宣言する', () => {
    expect(_AUTH_ROUTE_CACHE_POLICY).toBe('no-store');
  });

  it('/app/logout route layout は no-store cache policy を宣言する', () => {
    expect(LOGOUT_CACHE_POLICY).toBe('no-store');
  });
