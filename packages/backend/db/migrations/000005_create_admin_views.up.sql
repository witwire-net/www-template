...
-- account_summaries: アカウント基本情報と紐づく passkey 件数を集約したビュー。
...
LEFT JOIN public.passkey_credentials p ON p.account_id = a.id
...
-- account_passkeys: アカウントとその passkey 詳細を結合したビュー。
...
SELECT
  ...
  p.transports
FROM public.accounts a
JOIN public.passkey_credentials p ON p.account_id = a.id;
