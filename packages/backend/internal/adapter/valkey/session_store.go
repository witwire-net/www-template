...
func (s *SessionStore) SaveSession(ctx context.Context, sessionID string, accountID domain.AccountID, metadata application.SessionMetadata, ttl time.Duration) error {
...
func (s *SessionStore) ListSessions(ctx context.Context, accountID domain.AccountID) ([]application.SessionMetadata, error) {
	idxKey := s.store.Key("auth", "account-sessions", accountID.String())
...
func (s *SessionStore) RevokeSession(ctx context.Context, accountID domain.AccountID, sessionID string) error {
	key := s.store.Key("auth", "session", sessionID)
...
func (s *SessionStore) RevokeOthers(ctx context.Context, accountID domain.AccountID, currentSessionID string) ([]string, error) {
	idxKey := s.store.Key("auth", "account-sessions", accountID.String())
	sessionIDs, err := s.store.client.SMembers(ctx, idxKey).Result()
	if err != nil {
		return nil, domain.ErrAuthStoreUnavailable
...
func (s *SessionStore) RevokeAllForAccount(ctx context.Context, accountID domain.AccountID) error {
	idxKey := s.store.Key("auth", "account-sessions", accountID.String())
	sessionIDs, err := s.store.client.SMembers(ctx, idxKey).Result()
	if err != nil {
		return domain.ErrAuthStoreUnavailable
