...
var _ adminapplication.AdminAccountRepository = (*AccountRepository)(nil)
var _ adminapplication.AdminAccountSearchRepository = (*AccountRepository)(nil)
...
//   - adminapplication.AdminAccountRepository と AdminAccountSearchRepository port を実装し、application 層へ GORM record や DB error を公開しない。
...
//
// 戻り値:
//   - adminapplication.AdminAccountRecord: 永続化済み Account root snapshot。
...
// record.AuditCompletion は AdminAuditService が domain.AdminAuditEvent で作った success outcome で、account 作成 commit と同じ transaction で保存する。
func (r *AccountRepository) CreateAccountWithAuditTarget(ctx context.Context, record adminapplication.AdminAccountCreationRecord) (adminapplication.AdminAccountRecord, error) {
	// Step 1: repository または DB handle が欠けている場合、transaction を開始せず application error に畳む。
	if r == nil || r.db == nil {
		return adminapplication.AdminAccountRecord{}, adminapplication.ErrAdminAccountRepositoryUnavailable
	}

	// Step 2: 戻り値は transaction closure 内でだけ設定し、rollback 時に未保存 snapshot が返らないようにする。
	var created adminapplication.AdminAccountRecord
	...
	if err != nil {
		return adminapplication.AdminAccountRecord{}, err
	}
...
// 成功時は Product Account 要約 read model と次ページ cursor を返し、DB failure は ErrAdminAccountRepositoryUnavailable に畳む。
func (r *AccountRepository) SearchAccounts(ctx context.Context, query adminapplication.AdminAccountSearchQuery) (adminapplication.AdminAccountSearchRepositoryResult, error) {
	// Step 1: repository または DB handle が欠けている場合、検索 query を開始せず application error に畳む。
	if r == nil || r.db == nil {
		return adminapplication.AdminAccountSearchRepositoryResult{}, adminapplication.ErrAdminAccountRepositoryUnavailable
	}
...
// 成功時は admin_view.account_summaries の snapshot を返し、対象不在は ErrAdminAccountSearchNotFound に畳む。
func (r *AccountRepository) FindAccountByID(ctx context.Context, accountID string) (adminapplication.AdminAccountSummaryRecord, error) {
	// Step 1: repository または DB handle が欠けている場合、detail query を開始せず application error に畳む。
	if r == nil || r.db == nil {
		return adminapplication.AdminAccountSummaryRecord{}, adminapplication.ErrAdminAccountRepositoryUnavailable
	}
...
func createAccountRoot(ctx context.Context, tx *gorm.DB, record adminapplication.AdminAccountCreationRecord) (adminapplication.AdminAccountRecord, error) {
	...
	if duplicate {
		return adminapplication.AdminAccountRecord{}, adminapplication.ErrAdminAccountDuplicateEmail
	}
	...
	if err := tx.WithContext(ctx).Create(&account).Error; err != nil {
		return adminapplication.AdminAccountRecord{}, mapAccountMutationError(err)
	}
...
func accountEmailExists(ctx context.Context, tx *gorm.DB, email string) (bool, error) {
	...
	if err := tx.WithContext(ctx).Model(&accountRecord{}).Where("email = ?", email).Count(&count).Error; err != nil {
		return false, adminapplication.ErrAdminAccountRepositoryUnavailable
	}
...
func accountRecordFromDomain(record adminapplication.AdminAccountCreationRecord) accountRecord {
...
func updateAccountSetting(ctx context.Context, tx *gorm.DB, record adminapplication.AdminAccountCreationRecord) error {
	...
	if result.Error != nil {
		return adminapplication.ErrAdminAccountRepositoryUnavailable
	}
...
func bindAuditTarget(ctx context.Context, tx *gorm.DB, record adminapplication.AdminAccountCreationRecord) error {
	// Step 1: audit ID は空白だけの入力を拒否し、監査 intent なしの account mutation を rollback する。
	auditID := strings.TrimSpace(record.AuditID)
	if auditID == "" {
		return adminapplication.ErrAdminAccountAuditNotFound
	}
...
func accountRecordToApplicationRecord(account accountRecord, record adminapplication.AdminAccountCreationRecord) adminapplication.AdminAccountRecord {
	// Step 1: application DTO は DB column tag を持たない primitive snapshot として組み立てる。
	return adminapplication.AdminAccountRecord{
...
func mapAccountMutationError(err error) error {
	...
	if errors.Is(err, gorm.ErrDuplicatedKey) {
		return adminapplication.ErrAdminAccountDuplicateEmail
	}
...
func accountSearchRecordsToApplicationResult(records []accountSummaryRecord, limit int32) adminapplication.AdminAccountSearchRepositoryResult {
	...
	accounts := make([]adminapplication.AdminAccountSummaryRecord, 0, len(visibleRecords))
	...
	return adminapplication.AdminAccountSearchRepositoryResult{Accounts: accounts, NextCursor: nextCursor}
}

func accountSummaryRecordToApplicationRecord(record accountSummaryRecord) adminapplication.AdminAccountSummaryRecord {
	...
	return adminapplication.AdminAccountSummaryRecord{AccountID: record.ID, Email: record.Email, Status: record.Status, PasskeyCount: passkeyCount, CreatedAt: record.CreatedAt}
}
