...
var _ adminapplication.AdminAuditRepository = (*AuditRepository)(nil)

// AuditRepository は Admin audit intent/outcome を admin.audit_events に保存する PostgreSQL adapter である。
//
// 役割:
//   - application 層の AdminAuditRepository port を実装し、GORM record や SQL detail を外へ出さない。
...
// NewAuditRepository は Admin audit repository を構築する。
//
...
func (r *AuditRepository) RecordAuditIntent(ctx context.Context, record adminapplication.AdminAuditIntentRecord) (adminapplication.AdminAuditRecord, error) {
	// Step 1: repository または DB handle が欠ける場合は監査なし mutation を防ぐため内部 error に畳む。
	if r == nil || r.db == nil {
		return adminapplication.AdminAuditRecord{}, adminapplication.ErrAdminAuditInternal
	}
...
func (r *AuditRepository) FindAudit(ctx context.Context, auditID string) (adminapplication.AdminAuditRecord, error) {
	...
	if err := r.db.WithContext(ctx).Where("id = ?", auditID).First(&audit).Error; err != nil {
		return adminapplication.AdminAuditRecord{}, mapAuditRepositoryError(err)
	}
	return audit.toApplicationRecord(), nil
}

// CompleteAudit は pending audit event を succeeded または failed outcome へ更新する。
func (r *AuditRepository) CompleteAudit(ctx context.Context, record adminapplication.AdminAuditCompletionRecord) (adminapplication.AdminAuditRecord, error) {
...
func auditEventRecordFromIntent(record adminapplication.AdminAuditIntentRecord, auditID string) auditEventRecord {
...
func auditCompletionUpdates(record adminapplication.AdminAuditCompletionRecord) map[string]any {
...
func (r auditEventRecord) toApplicationRecord() adminapplication.AdminAuditRecord {
	...
	return adminapplication.AdminAuditRecord{AuditID: r.ID, OperatorID: r.OperatorID, Action: r.Action, TargetType: "account", TargetID: r.TargetAccountID, RequestID: r.RequestID, DetailsJSON: r.Metadata, Outcome: r.Outcome, StableErrorCode: stableErrorCode, OccurredAt: r.CreatedAt, CompletedAt: r.CompletedAt}
}

func mapAuditRepositoryError(err error) error {
	// Step 1: not found も復元不能な監査状態として扱い、外部へ DB 詳細を出さない抽象 error に畳む。
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return adminapplication.ErrAdminAuditInternal
	}
	return adminapplication.ErrAdminAuditInternal
}
