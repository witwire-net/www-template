...
// challengeKey が空文字列の場合は clientDataJSON から自己解決する（後方互換性のため）。
func (p *webAuthnProvider) FinishRegistration(ctx context.Context, challengeKey string, accountID string, credential application.WebAuthnAttestationCredentialDTO) (credentialHandle string, credData domain.WebAuthnCredentialData, err error) {
	lookupKey := challengeKey
	if lookupKey == "" {
		derivedKey, deriveErr := challengeKeyFromClientDataJSON(credential.Response.ClientDataJSON)
		if deriveErr != nil {
