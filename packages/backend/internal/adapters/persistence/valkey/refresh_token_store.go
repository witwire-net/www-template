...
func (s *RefreshTokenStore) Save(ctx context.Context, hash string, record application.RefreshTokenRecord, ttl time.Duration) error {
	...
	// インデックスに追加
	idxKey := s.store.Key("auth", "refresh_index", record.AccountID, record.Fingerprint)
...
func (s *RefreshTokenStore) RevokeAllForFingerprint(ctx context.Context, accountID, fingerprint string) error {
	idxKey := s.store.Key("auth", "refresh_index", accountID, fingerprint)
...
func (s *RefreshTokenStore) RevokeBySessionID(ctx context.Context, accountID, sessionID string) error {
	// アカウント単位の全インデックスをスキャンする簡易実装。
	// 実運用では prefix スキャンを利用する。
	pattern := s.store.Key("auth", "refresh_index", accountID, "*")
