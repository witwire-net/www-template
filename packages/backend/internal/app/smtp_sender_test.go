...
func TestSecureTransportRejectsServerWithoutSTARTTLS(t *testing.T) {
	...
	sender := NewSMTPSender(types.InfraConfig{
		SMTP: types.SMTPConfig{Host: "127.0.0.1", Port: port, SecureTransport: true},
		Mail: types.MailConfig{FromAddress: "test@example.com"},
	})
