...
func appAuthMiddleware(cfg types.Config, auth *usecases.AuthService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if auth == nil {
			...
			writeAuthFailure(c, usecases.ErrUnauthenticated)
			return
		}
...
func writeAuthFailure(c *gin.Context, err error) {
	...
	switch {
	case errors.Is(err, usecases.ErrSessionExpired):
		classification = openapi.SessionExpired
	case errors.Is(err, usecases.ErrInternalError):
...
func isNoStoreAuthPath(path string) bool {
	return strings.HasPrefix(path, "/api/v1/auth/") || path == "/api/v1/app/auth/logout"
}
