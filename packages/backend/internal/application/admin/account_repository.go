...
var (
	// ErrAdminAccountRepositoryUnavailable は Admin account repository が Account root または Admin schema を安全に更新できない場合に返す error である。
	//
	// 役割:
	...
	ErrAdminAccountRepositoryUnavailable = errors.New("admin account repository unavailable")

	// ErrAdminAccountDuplicateEmail は canonical AccountEmail が既存 Account root と重複した場合に返す error である。
	//
	// 役割:
	...
	ErrAdminAccountDuplicateEmail = errors.New("admin account duplicate email")

	// ErrAdminAccountAuditNotFound は account 作成 transaction に紐づける pending audit event が存在しない場合に返す error である。
	//
	// 役割:
	...
	ErrAdminAccountAuditNotFound = errors.New("admin account audit not found")

	// ErrAdminAccountSearchNotFound は Admin account read model に対象 account が存在しない場合に返す error である。
	//
	// 役割:
	...
	ErrAdminAccountSearchNotFound = errors.New("admin account search not found")
)

// AdminAccountRepository は Admin account creation が Product Account root と Admin audit target を同一 transaction で扱うための port である。
//
// 役割:
//   - application layer が GORM、Postgres、generated OpenAPI DTO、HTTP adapter 型に依存しないよう、domain.Account と primitive DTO だけを公開する。
//   - CreateAccountWithAuditTarget は domain.NewAdminCreatedAccount などで構築済みの Account root を保存し、同じ DB transaction 内で admin.audit_events の target と success outcome を関連付ける。
//   - repository 実装は public.accounts / public.account_settings と Admin-owned schema の audit table を同一 commit 境界に置く。
//
// 使用例:
//
//	created, err := accounts.CreateAccountWithAuditTarget(ctx, AdminAccountCreationRecord{Account: account, AuditID: audit.AuditID, AuditCompletion: completion})
//	if err != nil {
//		return err
//	}
//	_ = created
...
// AdminAccountSearchRepository は Admin account search use case が検証済み query だけで Product Account read model を取得するための port である。
//
// 役割:
...
// 使用例:
//
//	result, err := accounts.SearchAccounts(ctx, AdminAccountSearchQuery{Limit: 25})
//	if err != nil {
//		return err
//	}
//	_ = result
type AdminAccountSearchRepository interface {
	SearchAccounts(ctx context.Context, query AdminAccountSearchQuery) (AdminAccountSearchRepositoryResult, error)
	FindAccountByID(ctx context.Context, accountID string) (AdminAccountSummaryRecord, error)
}

// AdminAccountCreationRecord は account 作成 transaction へ渡す application DTO である。
//
// 役割:
...
//   - AuditCompletion は AdminAuditService が domain.AdminAuditEvent から作った success outcome で、account 作成 commit と同じ transaction で保存する。
//   - adapter 型や generated 型を含めず、port purity と Clean Architecture の境界を守る。
type AdminAccountCreationRecord struct {
	Account         domain.Account
	AuditID         string
	AuditCompletion AdminAuditCompletionRecord
}

// AdminAccountRecord は Admin account repository が永続化後に返す Account root snapshot である。
//
// 役割:
...
// AdminAccountSearchQuery は repository へ渡す検証済み account search query である。
//
// 役割:
...
type AdminAccountSearchQuery struct {
	Email  string
	Cursor string
	Limit  int32
}

// AdminAccountSummaryRecord は Admin account search repository が返す account 要約 read model である。
//
// 役割:
...
type AdminAccountSummaryRecord struct {
	AccountID    string
	Email        string
	Status       string
	PasskeyCount int32
...
// AdminAccountSearchRepositoryResult は repository search の結果 DTO である。
//
// 役割:
...
type AdminAccountSearchRepositoryResult struct {
	Accounts   []AdminAccountSummaryRecord
	NextCursor string
}
