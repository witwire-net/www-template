...
func TestAdminAuditUseCaseRecordsPendingIntentBeforeMutation(t *testing.T) {
	...
	repository := newFakeAdminAuditRepository()
	service := mustNewAdminAuditService(t, repository)

	// Step 2: account creation use case 予定の入力に近い operator/action/target correlation を渡す。
	stored, err := service.RecordMutationIntent(context.Background(), AdminAuditIntentInput{
...
func TestAdminAuditUseCaseCompletesFailedOutcomeThroughDomainEvent(t *testing.T) {
	...
	repository := newFakeAdminAuditRepository()
	repository.current = AdminAuditRecord{AuditID: "audit-1", Outcome: "pending"}
	service := mustNewAdminAuditService(t, repository)

	// Step 2: mixed-case の stable error code を渡し、canonical 化と failed transition を domain.AdminAuditEvent.MarkFailed に委譲する。
	stored, err := service.CompleteMutationFailed(context.Background(), AdminAuditFailureInput{
		AuditID:         "audit-1",
...
func TestAdminAuditUseCaseCompletesSucceededOutcomeThroughDomainEvent(t *testing.T) {
	...
	repository := newFakeAdminAuditRepository()
	repository.current = AdminAuditRecord{AuditID: "audit-2", Outcome: "pending"}
	service := mustNewAdminAuditService(t, repository)

	// Step 2: success 用 API を呼び、domain.AdminAuditEvent.MarkSucceeded が completed timestamp を必須にする境界を通す。
	stored, err := service.CompleteMutationSucceeded(context.Background(), AdminAuditCompletionInput{AuditID: "audit-2"})
	if err != nil {
...
func TestAdminAuditUseCaseFailsClosedWhenIntentCannotBeRecorded(t *testing.T) {
	...
	// Step 2: repository error は詳細を露出せず ErrAdminAuditInternal に写像される。
	_, err := service.RecordMutationIntent(context.Background(), AdminAuditIntentInput{Action: "accounts:create"})
...
func TestAdminAuditUseCaseRejectsAlreadyCompletedAudit(t *testing.T) {
	...
	repository := newFakeAdminAuditRepository()
	repository.current = AdminAuditRecord{AuditID: "audit-3", Outcome: "succeeded", CompletedAt: &completedAt}
	service := mustNewAdminAuditService(t, repository)

	// Step 2: 完了済み audit の再完了は ErrAdminAuditBadRequest になり、repository の CompleteAudit へ進まないことを確認する。
	_, err := service.CompleteMutationSucceeded(context.Background(), AdminAuditCompletionInput{AuditID: "audit-3"})
	if !errors.Is(err, ErrAdminAuditBadRequest) {
		t.Fatalf("expected ErrAdminAuditBadRequest, got %v", err)
	}
...
func TestAdminAuditUseCaseRejectsFailedOutcomeWithoutStableErrorCode(t *testing.T) {
	...
	repository := newFakeAdminAuditRepository()
	repository.current = AdminAuditRecord{AuditID: "audit-4", Outcome: "pending"}
	service := mustNewAdminAuditService(t, repository)

	// Step 2: failed outcome に必須の stable error code を空にし、domain.AdminAuditEvent.MarkFailed による拒否へ委譲されることを確認する。
	_, err := service.CompleteMutationFailed(context.Background(), AdminAuditFailureInput{AuditID: "audit-4"})
	if !errors.Is(err, ErrAdminAuditBadRequest) {
		t.Fatalf("expected ErrAdminAuditBadRequest, got %v", err)
	}
	if repository.completeCalls != 0 {
...
func mustNewAdminAuditService(t *testing.T, repository *fakeAdminAuditRepository) *AdminAuditService {
	t.Helper()

	// Step 1: test 用 clock は固定し、application code が time.Now に依存しないことを検証可能にする。
	service, err := NewAdminAuditService(repository, func() time.Time { return adminAuditUseCaseNow })
	if err != nil {
...
func assertCompletedAuditRecord(t *testing.T, record AdminAuditRecord, outcome string, stableErrorCode string) {
	t.Helper()
...
func assertCompletedAuditCommand(
	t *testing.T,
	command AdminAuditCompletionRecord,
	auditID string,
	outcome string,
...
type fakeAdminAuditRepository struct {
	current        AdminAuditRecord
	recordedIntent AdminAuditIntentRecord
	completed      AdminAuditCompletionRecord
	recordError    error
	findError      error
	completeError  error
	completeCalls  int
...
func newFakeAdminAuditRepository() *fakeAdminAuditRepository {
	// Step 1: default fake は audit ID を返せる pending repository として初期化し、各 test が必要な error だけ上書きする。
	return &fakeAdminAuditRepository{current: AdminAuditRecord{AuditID: "audit-1", Outcome: "pending"}}
}

func (r *fakeAdminAuditRepository) RecordAuditIntent(ctx context.Context, record AdminAuditIntentRecord) (AdminAuditRecord, error) {
...
func (r *fakeAdminAuditRepository) FindAudit(ctx context.Context, auditID string) (AdminAuditRecord, error) {
	...
	if r.findError != nil {
		return AdminAuditRecord{}, r.findError
	}
...
func (r *fakeAdminAuditRepository) CompleteAudit(ctx context.Context, record AdminAuditCompletionRecord) (AdminAuditRecord, error) {
	...
	if r.completeError != nil {
		return AdminAuditRecord{}, r.completeError
	}
