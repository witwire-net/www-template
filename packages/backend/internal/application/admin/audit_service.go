...
var (
	// ErrAdminAuditInternal は Admin audit 境界で repository、clock、または復元済み state が不正な場合に返す application error である。
	// adapter は内部詳細や永続化 error を露出せず、fail-close な 5xx 系応答へ変換する。
	ErrAdminAuditInternal = errors.New("admin audit internal")

	// ErrAdminAuditBadRequest は audit outcome を domain rule で完了できない入力または状態に対して返す application error である。
	// 二重完了、不正 outcome、不正 stable error code などを handler に domain 型を露出せず伝えるために使う。
	ErrAdminAuditBadRequest = errors.New("admin audit bad request")
)

// AdminAuditRepository は Admin mutation audit の intent 記録と outcome 完了を永続化 adapter へ委譲する port である。
//
// 役割:
...
//
// 戻り値:
//   - AdminAuditRecord: 永続化後の audit snapshot。audit ID や outcome を後続処理で参照する。
//   - error: repository が intent または outcome を保存できない場合に返す。use case は ErrAdminAuditInternal に写像する。
type AdminAuditRepository interface {
	RecordAuditIntent(ctx context.Context, record AdminAuditIntentRecord) (AdminAuditRecord, error)
	FindAudit(ctx context.Context, auditID string) (AdminAuditRecord, error)
	CompleteAudit(ctx context.Context, record AdminAuditCompletionRecord) (AdminAuditRecord, error)
}

// AdminAuditProjector は完了済み Admin audit event を検索基盤へ投影する port である。
//
// 役割:
...
type AdminAuditProjector interface {
	ProjectAdminAuditEvent(ctx context.Context, record AdminAuditProjectionRecord) error
}

// AdminAuditProjectionFailureObserver は audit projection の失敗を観測可能にする port である。
//
// 役割:
...
type AdminAuditProjectionFailureObserver interface {
	ObserveAdminAuditProjectionFailure(ctx context.Context, auditID string, err error)
}

// AdminAuditService は Admin mutation の intent 作成と success/failed outcome 完了を集約する application use case である。
//
// 役割:
...
// 使用例:
//
//	audit, err := NewAdminAuditService(repository, clock)
//	if err != nil {
//		return err
//	}
...
//		return err
//	}
//	_, err = audit.CompleteMutationSucceeded(ctx, AdminAuditCompletionInput{AuditID: intent.AuditID})
...
// AdminAuditIntentInput は mutation 開始前に記録する audit intent の入力 DTO である。
//
// 役割:
...
type AdminAuditIntentInput struct {
	OperatorID  string
	Action      string
	TargetType  string
	TargetID    string
...
// AdminAuditIntentRecord は repository へ渡す pending audit intent の保存 DTO である。
//
// 役割:
...
type AdminAuditIntentRecord struct {
	OperatorID      string
	Action          string
	TargetType      string
	TargetID        string
...
// AdminAuditCompletionInput は succeeded outcome へ完了する audit event の入力 DTO である。
//
// AuditID は RecordMutationIntent が返した永続化済み audit event の識別子である。
// 戻り値は repository が更新後に返す AdminAuditRecord で、失敗時は application error を返す。
type AdminAuditCompletionInput struct {
	AuditID string
}

// AdminAuditFailureInput は failed outcome へ完了する audit event の入力 DTO である。
//
...
type AdminAuditFailureInput struct {
	AuditID         string
	StableErrorCode string
}

// AdminAuditCompletionRecord は repository へ渡す completed audit outcome の保存 DTO である。
//
// 役割:
...
type AdminAuditCompletionRecord struct {
	AuditID         string
	Outcome         string
	StableErrorCode string
	CompletedAt     time.Time
...
// AdminAuditRecord は repository から application use case へ返す audit snapshot DTO である。
//
// 役割:
...
// AdminAuditProjectionRecord は Admin audit event を検索 index へ投影するための application DTO である。
//
// 役割:
...
type AdminAuditProjectionRecord struct {
	AuditID         string
	OperatorID      string
	Action          string
	TargetType      string
...
// NewAdminAuditService は Admin audit use case を生成する。
//
// 引数:
...
//
// 戻り値:
//   - *AdminAuditService: 検証済み依存を保持する use case service。
//   - error: 必須依存が欠ける場合は ErrAdminAuditInternal。
func NewAdminAuditService(audits AdminAuditRepository, clock func() time.Time) (*AdminAuditService, error) {
	// Step 1: 永続化 port または clock が欠けると未監査 mutation や非決定的時刻になり得るため、構築時に拒否する。
	if audits == nil || clock == nil {
		return nil, ErrAdminAuditInternal
	}

	// Step 2: 検証済み依存だけを保持し、以後の use case method が同じ Admin audit 境界を共有する。
	return &AdminAuditService{audits: audits, clock: clock}, nil
}
...
// 戻り値は repository が保存した AdminAuditRecord であり、保存失敗時は ErrAdminAuditInternal を返して mutation 開始を止める。
func (s *AdminAuditService) RecordMutationIntent(ctx context.Context, input AdminAuditIntentInput) (AdminAuditRecord, error) {
	// Step 1: 呼び出し元 context が既に中断済みなら、永続化と後続 mutation を開始しない。
	if err := ctx.Err(); err != nil {
		return AdminAuditRecord{}, ErrAdminAuditInternal
	}
...
// 戻り値は更新後の AdminAuditRecord で、二重完了や不正な復元 state は ErrAdminAuditBadRequest、保存失敗は ErrAdminAuditInternal を返す。
func (s *AdminAuditService) CompleteMutationSucceeded(ctx context.Context, input AdminAuditCompletionInput) (AdminAuditRecord, error) {
...
func (s *AdminAuditService) BuildMutationSucceededCompletion(input AdminAuditCompletionInput) (AdminAuditCompletionRecord, error) {
	...
	if err != nil {
		return AdminAuditCompletionRecord{}, mapAdminAuditDomainError(err)
	}
...
// 戻り値は更新後の AdminAuditRecord で、stable error code が不正な場合は ErrAdminAuditBadRequest を返す。
func (s *AdminAuditService) CompleteMutationFailed(ctx context.Context, input AdminAuditFailureInput) (AdminAuditRecord, error) {
...
func (s *AdminAuditService) completeAudit(
	ctx context.Context,
	auditID string,
...
func reconstituteAdminAuditEvent(record AdminAuditRecord) (domain.AdminAuditEvent, error) {
...
func completionRecordFromEvent(auditID string, event domain.AdminAuditEvent) (AdminAuditCompletionRecord, error) {
	// Step 1: domain method 完了後の completedAt を取得し、nil は不整合として内部 error にする。
	completedAt := event.CompletedAt()
	if completedAt == nil {
		return AdminAuditCompletionRecord{}, ErrAdminAuditInternal
	}

	// Step 2: repository が保存する primitive DTO へ outcome、stable code、完了時刻を変換する。
	return AdminAuditCompletionRecord{
		AuditID:         auditID,
		Outcome:         string(event.Outcome()),
...
func mapAdminAuditDomainError(err error) error {
	...
	if errors.Is(err, domain.ErrInvalidAdminAuditOutcome) ||
		...
		return ErrAdminAuditBadRequest
	}

	// Step 2: 想定外 error は詳細を露出せず内部 error として扱う。
	return ErrAdminAuditInternal
}
