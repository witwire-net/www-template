...
// AccountAuthRepository は Auth use case が Account.Auth projection を取得・更新するための port である。
//
...
type RefreshTokenStore interface {
	...
	RevokeAllForFingerprint(ctx context.Context, accountID domain.AccountID, fingerprint string) error
	// RevokeBySessionID は指定されたセッション ID に紐づく全リフレッシュトークンを失効する。
	RevokeBySessionID(ctx context.Context, accountID domain.AccountID, sessionID string) error
}

// SessionStore はセッションメタデータの保存・一覧・失効を抽象化するポート。
type SessionStore interface {
	// SaveSession はセッションメタデータを保存する。
	SaveSession(ctx context.Context, sessionID string, accountID domain.AccountID, metadata SessionMetadata, ttl time.Duration) error
	...
	ListSessions(ctx context.Context, accountID domain.AccountID) ([]SessionMetadata, error)
	// RevokeSession は特定セッションを削除する。
	RevokeSession(ctx context.Context, accountID domain.AccountID, sessionID string) error
	// RevokeOthers は現在のセッション以外を全て削除し、削除した session ID のスライスを返す。
	RevokeOthers(ctx context.Context, accountID domain.AccountID, currentSessionID string) ([]string, error)
...
func NewAuthService(stateRepo AuthStateRepository, accountRepo AccountAuthRepository, recoverySender AccountRecoverySender, invitationRegistrar InvitationPasskeyRegistrar, clock func() time.Time, policy id.AuthIDPolicy, authConfig config.AuthConfig) *AuthService {
	if clock == nil {
