...
type TokenService struct {
	...
	accountRepo  AccountAuthRepository
...
func NewTokenService(refreshStore RefreshTokenStore, sessionStore SessionStore, accountRepo AccountAuthRepository, cfg config.AuthConfig, clock func() time.Time, policy id.AuthIDPolicy) *TokenService {
	if clock == nil {
...
// 発行されたセッション ID も返す。
func (s *TokenService) Issue(ctx context.Context, accountID domain.AccountID, fingerprint, deviceName, ipHash, existingSessionID string) (accessToken, refreshToken, sessionID string, err error) {
...
func (s *TokenService) Refresh(ctx context.Context, refreshToken, clientIP, userAgent string) (newAccessToken, newRefreshToken string, err error) {
	newAccessToken, newRefreshToken, _, err = s.RefreshWithAccountID(ctx, refreshToken, clientIP, userAgent)
	return newAccessToken, newRefreshToken, err
}

// RefreshWithAccountID はリフレッシュトークンを rotation し、合成処理用に確定済み AccountID も返す。
func (s *TokenService) RefreshWithAccountID(ctx context.Context, refreshToken, clientIP, userAgent string) (newAccessToken, newRefreshToken string, accountID domain.AccountID, err error) {
	hash := hashToken(refreshToken)
	...
	return newAccessToken, newRefreshToken, record.AccountID, nil
}
...
func (s *TokenService) checkAccountSuspended(ctx context.Context, accountID domain.AccountID, sessionID string, issuedAt time.Time) error {
...
func (s *TokenService) RevokeSession(ctx context.Context, accountID domain.AccountID, sessionID string) error {
...
func (s *TokenService) RevokeAllForAccount(ctx context.Context, accountID domain.AccountID) error {
