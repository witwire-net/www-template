...
func TestAdminAccountSearchRejectsOutOfRangeLimitBeforeRepository(t *testing.T) {
	...
	repository := &fakeAccountSearchRepository{}
	service := mustNewAccountSearchService(t, repository)
	limit := int32(0)

	// Step 2: limit=0 を渡し、application use case が repository より前に validation error を返すことを確認する。
	_, err := service.SearchAccounts(context.Background(), AccountSearchInput{Limit: &limit, RequestID: "req-search-1"})
	if !errors.Is(err, ErrAccountSearchInvalidInput) {
		t.Fatalf("expected ErrAccountSearchInvalidInput, got %v", err)
	}
...
func TestAdminAccountSearchUsesDefaultLimitAndReturnsReadModel(t *testing.T) {
	...
	repository := &fakeAccountSearchRepository{result: AccountSearchRepositoryResult{Accounts: []AccountSummaryRecord{{
	...
	result, err := service.SearchAccounts(context.Background(), AccountSearchInput{Email: " customer@example.com ", RequestID: "req-search-2"})
	if err != nil {
...
func mustNewAccountSearchService(t *testing.T, repository AccountSearchRepository) *AccountSearchService {
	t.Helper()

	// Step 1: constructor 経由で service を作り、nil dependency validation と test 対象の条件を揃える。
	service, err := NewAccountSearchService(repository)
	if err != nil {
...
type fakeAccountSearchRepository struct {
	calls        int
	detailCalls  int
	query        AccountSearchQuery
	result       AccountSearchRepositoryResult
	detailResult AccountSummaryRecord
	err          error
}

func (r *fakeAccountSearchRepository) SearchAccounts(ctx context.Context, query AccountSearchQuery) (AccountSearchRepositoryResult, error) {
	// Step 1: fake でも context cancellation を尊重し、実 repository と同じ入口条件を保つ。
	if err := ctx.Err(); err != nil {
		return AccountSearchRepositoryResult{}, err
	}
	...
	r.query = query
	if r.err != nil {
		return AccountSearchRepositoryResult{}, r.err
	}
...
func (r *fakeAccountSearchRepository) FindAccountByID(ctx context.Context, accountID string) (AccountSummaryRecord, error) {
	// Step 1: fake でも context cancellation を尊重し、detail use case の入口条件を実 repository と揃える。
	if err := ctx.Err(); err != nil {
		return AccountSummaryRecord{}, err
	}

	// Step 2: detail 呼び出し回数を記録し、validation 後にだけ repository が呼ばれることを観測可能にする。
	r.detailCalls++
	if accountID == "" {
		return AccountSummaryRecord{}, ErrAccountSearchNotFound
	}
	if r.err != nil {
		return AccountSummaryRecord{}, r.err
	}
