...
// [UT-AUTH-BE-ERR-001] Invalid AccountID subject verification fails
func TestVerifyAccessTokenInvalidSubject(t *testing.T) {
	t.Parallel()

	secret := []byte("a-very-long-secret-key-for-hs256-jwt-signing-32bytes-min")
	now := time.Now().UTC()
	claims := NewClaims(AccountID("invalid-account-id"), "01ARZ3NDEKTSV4RRFFQ69G5FAW", "01ARZ3NDEKTSV4RRFFQ69G5FAX", now)
