...
func NewClaims(accountID AccountID, sessionID, tokenID string, now time.Time) Claims {
	return Claims{
...
func decodeAndValidateClaims(payloadB64 string, now time.Time) (Claims, error) {
	...
	accountID, err := NewAccountID(claims.Subject.String())
	if err != nil {
		return Claims{}, ErrInvalidSignature
	}
