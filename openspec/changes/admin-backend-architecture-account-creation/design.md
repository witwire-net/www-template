...
- DB / migration: Product DB 内 Admin-owned schema、operator / passkey / audit tables、least-privilege DB roles、migration pair rollback。
...
- Purpose / Responsibility: Admin schema persistence と Admin auth/session store を application ports として実装する。
...
- Non-Functional: least-privilege DB role と request ID audit correlation を維持する。
...
| IT-ADMIN-CONSOLE-BE-REG-016  | `[ADMIN-CONSOLE-BE-S059] Admin schema exists inside Product DB`                              | be    | REG      | migration up state。                                                            | `000007` up 適用後 schema/table を確認。                                                         | Admin-owned schema/tables が存在。                                                                                              |
...
| IT-ADMIN-CONSOLE-BE-REG-053  | `[ADMIN-CONSOLE-BE-S082] Admin schema migration rollback satisfies pair policy`              | be    | REG      | migration rollback pair。                                                       | `000007` up 後に 1 step rollback を実行。                                                        | Admin schema/grants が戻り、Product `public.accounts` は保持される。                                                            |
...
| IT-ADMIN-CONSOLE-BE-REG-022  | `[ADMIN-CONSOLE-BE-S061] Admin package ORM migration is not applied to Product DB`           | be    | REG      | Prisma migration exclusion。                                                    | migration command policy と DB migrations を確認。                                               | backend migration system のみ対象。                                                                                             |
