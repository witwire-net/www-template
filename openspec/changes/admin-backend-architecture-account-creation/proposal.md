...
- **BREAKING** Admin DB の物理分割を廃止し、Admin operator / audit / account management に必要な永続データは Product と同一 PostgreSQL DB 内の Admin-owned schema に保持する。
...
- `admin-console-be`: Modified。Admin 管理 API、同一 Product DB 上の Admin 永続化、監査、Account 作成、Admin 専用 Go binary / host、Product API 非露出、Admin surface も `/api/v1/*` path policy に従う要件へ更新する。`packages/backend/internal/domain/**` の concrete domain object と unit test を必須にする。
...
- DB: Admin DB 物理分割廃止、Product DB 内の Admin schema / tables / audit / operator / account management persistence、migration 管理境界。migration は `000007_create_admin_schema.up.sql` / `000007_create_admin_schema.down.sql` とする。
