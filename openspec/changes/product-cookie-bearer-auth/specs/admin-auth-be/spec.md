...
#### Scenario: Admin middleware が operator access Cookie を検証して request context に設定する (ADMIN-AUTH-BE-S057)

- **GIVEN** 有効な operator access Cookie と Admin Valkey session record が存在する
- **WHEN** protected Admin API を呼び出す
- **THEN** Admin backend middleware は現在の operator、role、session ID、CSRF binding を context に設定する
- **AND** Product account session validator と `Authorization: Bearer` header は使用されない
...
- **GIVEN** request が有効な operator access Cookie を持つが、CSRF token が別 session に束縛されている
...
- **GIVEN** request が operator access Cookie と CSRF token を持たないが、Origin は allowlist に含まれる
...
Admin access Cookie と Admin refresh Cookie は Admin ドメインに対して `HttpOnly; Secure; SameSite=Lax` と限定 Path を含む安全属性で設定されなければならない（MUST）。Admin frontend と Admin backend は同一 Admin ドメインでホストされるため、production cookie は第三者 site 用 cookie policy に依存してはならない（MUST NOT）。development 以外で insecure cookie を設定してはならない（MUST NOT）。Admin auth response body は operator/session metadata と CSRF token を含めてよいが、operator accessToken または operator refreshToken 平文を含めてはならない（MUST NOT）。全 Admin auth cookie response は no-store header を含まなければならない（SHALL）。Product Web account access Cookie / refresh Cookie は Admin operator Cookie として使用してはならない（MUST NOT）。
...
- **THEN** access Cookie と refresh Cookie の `Set-Cookie` は `HttpOnly; Secure; SameSite=Lax` と限定 Path を含む
- **AND** response body は operator accessToken と refreshToken 平文を含まない
...
- **GIVEN** operator が有効な operator access Cookie、Admin session record、CSRF binding を持つ
- **WHEN** same-origin `GET /api/v1/auth/passkeys` を呼び出す
- **THEN** Admin backend は operator 自身の passkey credential 一覧を返す
- **AND** Product auth domain/application、Authorization header、package-local BFF route は使用されない
...
Admin operator passkey login と passkey registration は Admin API binary の same-origin `/api/v1/auth/*` surface で WebAuthn `userVerification="required"` を要求しなければならない（MUST）。Admin backend は assertion / attestation の user verification flag を検証し、user verification が成立しない credential を拒否しなければならない（MUST）。成功時は Admin OperatorAuth domain/application が operator access Cookie と Admin refresh Cookie を発行し、Product account auth domain/application を使用してはならない（MUST NOT）。

**Customer Context**

Admin Console は強権限操作を扱うため、device 所持だけでは不十分である。Admin operator auth は Product account auth と別に user verification を検証し、Cookie-only operator session を発行する必要がある。
...
#### Scenario: user verification ありの Admin assertion は operator Cookie session を発行する (ADMIN-AUTH-BE-S074)

- **GIVEN** WebAuthn assertion の `userVerification` flag が true で、operator credential が valid である
- **WHEN** Admin backend の `/api/v1/auth/passkey/finish` を呼び出す
- **THEN** Admin backend は HttpOnly access Cookie と HttpOnly refresh Cookie を設定する
- **AND** response body は operator/session metadata と CSRF token を含み、operator accessToken または refreshToken 平文を含まない

### Requirement: 初回起動セットアップは最初の admin オペレーターを作成する

Admin initial setup endpoints は Admin API binary の same-origin `/api/v1/auth/setup/*` surface として提供されなければならない（SHALL）。Setup flow は operator が 0 件であり、bootstrap enable flag、bootstrap secret hash、有効期限がすべて valid な場合だけ challenge を発行しなければならない（MUST）。Bootstrap secret 平文は DB、audit、log、trace、response body に保存または出力してはならない（MUST NOT）。Setup finish は Admin Operator domain object、Admin OperatorAuth domain object、Admin Valkey `admin:*` challenge、Admin schema transaction を使い、role=`admin` の最初の operator と passkey credential を作成しなければならない（MUST）。Setup finish 成功時は HttpOnly operator access Cookie、HttpOnly operator refresh Cookie、operator/session metadata、CSRF token を返し、operator accessToken または refreshToken 平文を body に含めてはならない（MUST NOT）。`packages/admin` は setup BFF route または server-side setup handler を所有してはならない（MUST NOT）。

**Customer Context**

初回 admin 作成は Admin surface の最初の trust anchor である。静的 frontend package や DB seed に secret logic を置かず、Admin backend の domain/application と fail-close config に集約する必要がある。setup は最初の operator session を作るため、通常 login と同じ Cookie-only boundary にそろえ、browser-readable operator token を返してはならない。
...
- **AND** HttpOnly operator access Cookie、HttpOnly operator refresh Cookie、operator/session metadata、CSRF token が発行される
...
Additional operator setup endpoints は Admin API binary の same-origin `/api/v1/auth/operator-setup/*` surface として提供されなければならない（SHALL）。Admin による operator 作成は setup token hash と expiry を Admin schema に保存し、setup token 平文を DB、audit、log、trace、response body に保存または出力してはならない（MUST NOT）。Operator setup start/finish は Admin backend が token hash、expiry、one-time consumption、WebAuthn challenge、passkey registration、Admin operator Cookie session 発行を検証し、finish 成功時は HttpOnly operator access Cookie、HttpOnly operator refresh Cookie、operator/session metadata、CSRF token を返し、operator accessToken または refreshToken 平文を body に含めてはならない（MUST NOT）。`packages/admin` は setup BFF route を所有してはならない（MUST NOT）。

**Customer Context**

追加された operator は passkey credential を持たないため、one-time setup token で初回登録を完了する。token secret と registration logic を静的 frontend に置かず、Admin backend に閉じることで漏洩と列挙を防ぐ。初回登録後の session も Cookie-only にすることで、setup token と operator token の両方が browser-readable state に残らない。
