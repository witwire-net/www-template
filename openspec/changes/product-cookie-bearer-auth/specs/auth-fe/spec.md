...
- FROM: `### Requirement: クライアントは JWT アクセストークンの有効期限を監視し自動更新する`
- TO: `### Requirement: クライアントは Cookie session と auth context selector で session を継続する`
...
- `/login` route で認証が成功した後の client auth state は、HttpOnly Cookie credential、server-issued `X-Auth-Context-Id` selector、session-bound CSRF token で `/api/v1/*` を利用できる Cookie session 契約に SHALL 接続される。
- `/login` route の client request は `credentialMode="cookie"` を SHALL 明示し、response body の bearer accessToken または refreshToken に依存してはならない（MUST NOT）。
...
#### Scenario: Web login は HttpOnly Cookie session と auth context selector と CSRF token を受け入れる (AUTH-FE-S051)

- **GIVEN** 利用者が `/login` で passkey sign-in を完了する
- **WHEN** API が Cookie mode の session response を返す
- **THEN** クライアントは bearer accessToken を保存せず、active auth context metadata、`authContextId`、CSRF token を memory state に保持して authenticated state へ遷移する
...
パスキーを紛失した利用者（kind=recovery）と、新端末でログインを有効にしたい利用者（kind=device-link）は、どちらも invite onboarding へ戻らずに、登録済みアカウントの passkey を安全に登録できる必要がある。登録成功後は Cookie session と auth context selector でログイン状態へ進み、JavaScript 可読 token を画面・state・telemetry に残さない必要がある。
...
- 登録成功後の client auth state は、HttpOnly Cookie credential、server-issued `X-Auth-Context-Id` selector、session-bound CSRF token で `/api/v1/*` を利用できる Cookie session 契約に SHALL 接続される。
- recovery/device-link registration request は `credentialMode="cookie"` を SHALL 明示し、response body の bearer accessToken または refreshToken に依存してはならない（MUST NOT）。
...
#### Scenario: 復旧登録成功時は Cookie session としてログインする (AUTH-FE-S052)

- **GIVEN** 利用者が recovery/device-link branch の passkey registration を完了する
- **WHEN** API が Cookie mode の session response を返す
- **THEN** クライアントは bearer accessToken を保存せず、active auth context metadata、`authContextId`、CSRF token を memory state に保持して authenticated state へ遷移する

### Requirement: クライアントは Cookie session と auth context selector で session を継続する

クライアントは Cookie session と auth context selector の有効性を server response と refresh endpoint で確認し、JavaScript から access credential を読まずに session を継続しなければならない（MUST）。

**Customer Context**

利用者はアプリ操作中に認証が突然切れてデータを失う体験を避けたい。Cookie session では access credential が HttpOnly であるため、frontend は JWT payload を読まず、API response、`X-Auth-Context-Id` selector、refresh endpoint を使って安全に session 継続可否を判断する必要がある。
...
- `authContextId` は server-issued selector として memory state にのみ保持し、全 Product protected request の `X-Auth-Context-Id` header に設定しなければならない（MUST）。
...
#### Scenario: 期限切れの Cookie session は refresh 後に API を再試行する (AUTH-FE-S045)

- **GIVEN** Cookie session の access credential が server-side で期限切れとして扱われる
- **WHEN** 保護された API 呼び出しが `session-expired` を返す
- **THEN** クライアントは same-origin credential request として `POST /api/v1/auth/refresh` を実行し、成功後に更新された `authContextId` と CSRF token で同じ API 呼び出しを 1 回だけ再試行する

#### Scenario: 既に期限切れの Cookie credential はリフレッシュ後に API を呼び出す (AUTH-FE-S024)

- **GIVEN** Cookie session の access credential が期限切れで refresh credential は有効である
- **WHEN** 保護された API 呼び出しが必要になる
- **THEN** クライアントは refresh 成功後に最新 `authContextId` と CSRF token を使って API 呼び出しを継続し、リフレッシュ失敗時は route intent を更新する

#### Scenario: Cookie mode の token はブラウザーから読める storage に保存されない (AUTH-FE-S046)

- **GIVEN** 利用者がログインして Cookie mode の session response を受け取る
- **WHEN** 認証 state がクライアントに保存される
- **THEN** bearer accessToken、refreshToken、Cookie value は永続ストレージに書き込まれず、`authContextId`、CSRF token、session metadata だけが memory state に保持される
...
クライアントは Product Web の Cookie credential container に紐づく server-issued auth context selector を使い、browser-readable credential を使わずに複数 account/session context を保持・切り替えなければならない（MUST）。

**Customer Context**

HttpOnly Cookie session は JavaScript から credential を読ませないことで安全性を高める。一方で、同一 origin の Cookie は browser 内で自動送信されるため、frontend memory に複数の bearer token を保持して切り替える方式とは性質が異なる。利用者が複数アカウントを安全に使い分けられるように、Product Web は server-owned auth context registry から受け取った opaque `authContextId` だけを selector として扱い、backend が credential との所属を検証した context だけで表示と API 呼び出しを行う必要がある。

**Requirement**

- クライアントは Product Web 上で bearer accessToken を複数保持して account switching に使ってはならない（MUST NOT）。
- ログイン成功、recovery registration 成功、bootstrap refresh 成功のいずれの場合も、クライアントは active auth context metadata、`activeAuthContextId`、switchable auth contexts を memory state に保持しなければならない（MUST）。
- 追加のログインが同じ browser context で成功した場合、クライアントは backend から返る switchable auth contexts を統合し、active auth context を新しい `authContextId` に切り替えなければならない（MUST）。
- UI は Product Cookie session で複数アカウント切り替えコントロールを表示し、選択中の `authContextId` を以後の protected request の `X-Auth-Context-Id` header に設定しなければならない（MUST）。
- logout は active auth context のみを対象とし、server-side revoke と Cookie / registry update response を受けた後、active context を消去または残りの context へ切り替えなければならない（MUST）。

#### Scenario: ログイン成功時は active auth context を置き換える (AUTH-FE-S048)

- **GIVEN** 利用者が Product Web で authenticated state にいる
- **WHEN** 同じ browser context で別のログインが成功する
- **THEN** クライアントは active auth context metadata を新しい `authContextId` に切り替え、ブラウザーから読める credential list を保持しない

#### Scenario: API 呼び出しは Cookie と auth context selector と CSRF token を使用する (AUTH-FE-S028)

- **GIVEN** Product Web が active auth context を保持している
- **WHEN** 保護された API が呼び出される
- **THEN** request は browser が送信する HttpOnly Cookie、memory state の `X-Auth-Context-Id`、CSRF token を使い、UI 上の bearer token switching に依存しない

#### Scenario: Product Web では selector ベースのアカウント切り替え UI を表示する (AUTH-FE-S049)

- **GIVEN** Product Web が authenticated state にいる
- **WHEN** 認証済みアプリ画面が表示される
- **THEN** UI は server-issued auth contexts だけを表示するアカウント切り替えコントロールを表示し、accessToken や Cookie value は表示しない

#### Scenario: logout は active auth context を失効する (AUTH-FE-S050)

- **GIVEN** Product Web が active auth context を保持している
- **WHEN** 利用者がログアウトする
- **THEN** クライアントは active auth context をサーバーと memory state の両方で失効し、残りの auth context がなければ未認証導線へ遷移する

#### Scenario: 全セッション消失時は未認証導線へ遷移する (AUTH-FE-S031)

- **GIVEN** active auth context が logout または失効によって存在しなくなる
- **WHEN** client の auth contexts が空になる
...
- 現在の Cookie session と active auth context が有効である間、`/*` 上の authenticated navigation は SHALL 継続する。
- 以前は有効だった session が expired または revoked と判定され、refresh でも継続できないとき、クライアントは利用者を `/session-expired` へ SHALL 遷移させる。
...
- `/logout` route は現在の active auth context を SHALL revoke し、クライアントが保持する該当 auth context metadata と CSRF token を消去し、残りの auth context がない場合は利用者を public route または login route の非認証状態へ戻す。
...
- logout / expiry handling でクライアントが参照する session ID、account ID、event ID、request ID、notification ID などの識別子が必要な箇所は ULID を SHALL 用い、opaque credential や cache key を ULID resource ID と混同してはならない。
...
- **GIVEN** 利用者が active auth context を持っている
- **WHEN** 利用者が `/logout` を開く
- **THEN** active auth context は revoke され、client memory の該当 metadata と CSRF token は消去され、残りの auth context がなければ利用者は signed in として振る舞わない public route または login route に到達する
...
認証済み Product Web 利用者は、active Product Cookie session、`X-Auth-Context-Id`、CSRF token によって、パスキー一覧・追加・削除・デバイスリンク送信を実行できなければならない（SHALL）。
...
- パスキー管理 API 呼び出しは same-origin credential request として HttpOnly Cookie を送信し、memory state の `authContextId` を `X-Auth-Context-Id` header に設定し、state-changing request には memory state の CSRF token を `X-CSRF-Token` header に設定しなければならない（MUST）。
...
- 管理ページは active Product Cookie session と auth context selector を必須とする認証済み surface であり、未認証アクセスは SHALL 拒否されなければならない。
...
- **THEN** `X-Auth-Context-Id` と CSRF header 付き Cookie request で削除 API が呼び出され、そのパスキーが一覧から削除され、残りは変化しない
...
- **THEN** UI は WebAuthn 再認証を求め、成功後に `X-Auth-Context-Id` と CSRF header 付き Cookie request で登録メールアドレスへデバイスリンクを送信し、送信完了・有効期限・共有禁止の案内を表示する
...
#### Scenario: Web auth state は bearer token と Cookie value を保存しない (AUTH-FE-S054)

- **GIVEN** `credentialMode="cookie"` の session response が返っている
- **WHEN** クライアントが auth state を更新する
- **THEN** bearer accessToken、refreshToken、HttpOnly Cookie value は state に保存されず、authContextId、CSRF token、session metadata だけが memory state に残る

### Requirement: suspended account は認証 UI で安全に案内される

顧客向け frontend は HTTP 403 の `AuthFailureResponse` として `error="account-suspended"` を受け取った場合、該当 account の Product Cookie auth context state を MUST 消去し、アカウントが利用できないこととサポート問い合わせ導線を SHALL 表示する。public login start や recovery request の段階では account existence や suspended 状態を UI に表示してはならない（MUST NOT）。
...
- **THEN** クライアントは session metadata と CSRF token を保存せず、account suspended 案内とサポート問い合わせ導線を表示する
...
- **THEN** クライアントは該当 account の Product Cookie auth context state を消去し、account suspended 案内へ遷移する
...
- **GIVEN** Product Web が active Product Cookie session を保持している
- **WHEN** その session の API 呼び出しだけが `account-suspended` を返す
- **THEN** クライアントは active session state を消去し、他の browser-readable session credential を保持または切り替えない
...
利用者は、どのデバイスやブラウザから自分のアカウントにアクセスされているかを把握したい。紛失した端末や公共 PC でのログインを忘れた場合、そのセッションをリモートで無効化できる必要がある。Product Cookie session では state-changing 操作を `X-Auth-Context-Id` と CSRF header 付き same-origin credential request として実行し、安全に session を管理する必要がある。
...
- 各デバイスには「ログアウト」アクションを提供し、クリック時に `DELETE /api/v1/sessions/{id}` を `X-Auth-Context-Id` と CSRF header 付き Cookie request として呼び出して該当セッションを無効化しなければならない（MUST）。
- デバイス管理ページには「他のすべてのデバイスをログアウト」ボタンを提供し、クリック時に `DELETE /api/v1/sessions/others` を `X-Auth-Context-Id` と CSRF header 付き Cookie request として呼び出して現在のセッション以外を一括無効化しなければならない（MUST）。
...
- デバイス一覧の取得および無効化操作は、active Product Cookie session と auth context selector を用いて認証済みリクエストとして実行しなければならない（MUST）。
...
#### Scenario: デバイス管理ページで特定デバイスをログアウトできる (AUTH-FE-S053)

- **GIVEN** 利用者がデバイス管理ページを開いている
- **WHEN** 利用者が特定デバイスの「ログアウト」アクションを実行する
- **THEN** `DELETE /api/v1/sessions/{id}` が `X-Auth-Context-Id` と CSRF header 付き Cookie request として呼び出され、該当デバイスのセッションが無効化される。一覧から該当デバイスが除去される

#### Scenario: デバイス管理ページで他のすべてのデバイスをログアウトできる (AUTH-FE-S036)

- **GIVEN** 利用者がデバイス管理ページを開いている
- **WHEN** 利用者が「他のすべてのデバイスをログアウト」ボタンをクリックする
- **THEN** `DELETE /api/v1/sessions/others` が `X-Auth-Context-Id` と CSRF header 付き Cookie request として呼び出され、現在のデバイスを除くすべてのセッションが無効化される。一覧からそれらのデバイスが除去され、現在のデバイスのみが残る
