...
#### Scenario: Web login は HttpOnly Cookie session と CSRF token を受け入れる (AUTH-FE-S051)

- **GIVEN** 利用者が `/login` で passkey sign-in を完了する
- **WHEN** API が Web Cookie mode の session response を返す
- **THEN** クライアントは bearer accessToken を保存せず、session metadata と CSRF token を memory state に保持して authenticated state へ遷移する
...
#### Scenario: 復旧登録成功時は Web Cookie session としてログインする (AUTH-FE-S052)
...
- **THEN** クライアントは same-origin credential request として `POST /api/v1/auth/refresh` を実行し、成功後に更新された CSRF token で同じ API 呼び出しを 1 回だけ再試行する
...
#### Scenario: Web Cookie mode の token はブラウザーから読める storage に保存されない (AUTH-FE-S046)
...
- **THEN** クライアントは session metadata、CSRF token、AccountSetting snapshot を memory state に反映し、authenticated state として表示する
...
HttpOnly Cookie session は JavaScript から credential を読ませないことで安全性を高める。一方で、同一 origin の Cookie は browser 内で自動送信されるため、frontend memory に複数の bearer token を保持して切り替える方式とは性質が異なる。利用者に誤ったアカウント切り替え体験を提供しないため、Product Web はサーバーが認識する 1 つの active Cookie session を基準に表示と API 呼び出しを行う必要がある。
...
- **THEN** クライアントは active session metadata を新しい session metadata に置き換え、ブラウザーから読める複数 session list を保持しない
...
#### Scenario: logout は active Cookie session を失効する (AUTH-FE-S050)

- **GIVEN** Product Web が active Web Cookie session を保持している
- **WHEN** 利用者がログアウトする
- **THEN** クライアントは active session をサーバーと memory state の両方で失効し、未認証導線へ遷移する
...
- `/logout` route は現在の active Web Cookie session を SHALL revoke し、クライアントが保持する session metadata と CSRF token を消去し、利用者を public route または login route の非認証状態へ戻す。
