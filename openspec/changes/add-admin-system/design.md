...
- Admin 専用 Valkey が利用可能であり、`ADMIN_VALKEY_URL` 環境変数で challenge / rate limit / lock 管理に使用すること。顧客向け Product auth runtime の Valkey 接続情報（`VALKEY_URL` または Product 専用 secret）とは別の接続情報を使い、Admin runtime は Product auth Valkey client を生成・操作してはならない
...
flowchart LR
    ...
    Container -->|Redis ADMIN_VALKEY_URL| AdminValkey[("Admin Valkey")]
    ProductBackend[Product Backend] -->|Redis VALKEY_URL| ProductValkey[("Product Auth Valkey")]
...
- Admin Valkey は `ADMIN_VALKEY_URL` を使用し、顧客向け Product auth runtime の Valkey 接続情報（`VALKEY_URL` または Product 専用 secret）とは別の secret / endpoint / credential として管理しなければならない。Admin コンテナには Product auth Valkey の接続文字列を注入してはならず、Redis logical DB 番号、ACL user、key prefix だけを分離境界として扱ってはならない。
- Admin runtime は Admin Valkey client のみを生成し、challenge / rate limit / lock の読み書きは `admin:*` prefix のみで行う。Product auth Valkey client を生成・注入・操作してはならず、Product auth の `auth:*` / `session:*` / `recovery:*` key は読み書きしてはならない。
- Local/dev compose は Product auth Valkey とは別に Admin 専用 `admin-valkey` service と volume を持たなければならない。`ADMIN_VALKEY_URL` は `admin-valkey` を指し、Product `VALKEY_URL` の `valkey` service を流用してはならない。
...
- Key Flows: `POST /api/admin/auth/passkey/start` で ULID `challengeId` と challenge を生成し、`ADMIN_VALKEY_URL` の Admin 専用 Valkey に `SETEX admin:webauthn:challenge:<challengeId> 300 <record-json>` で保存 → response に `challengeId` と WebAuthn options を返す → browser で `getAssertion()` → `POST /api/admin/auth/passkey/finish` で `challengeId` を受け取り、Admin 専用 Valkey から challenge record を `GETDEL` で取得・削除 → record の `type` / `operatorId` / `email` と credential owner を照合 → assertion 検証 → `sessionId` と `jti` を生成して `SETEX admin:session:<sessionId> 86400 <record-json>` → JWT 発行 → Set-Cookie。hook は JWT signature / exp と Valkey active session record の `sessionId` / `jti` を検証し、JWT role claim ではなく Admin DB の現在 role を毎 request で取得して認可に使う。logout は `admin:session:<sessionId>` を削除または revoked marker に置換し、盗難済み cookie を期限前に無効化する
...
- Security: JWT exp=24h, cookie=httpOnly/Secure/SameSite=Lax/Path=/, Valkey active session record exp=24h, logout/revocation check required, WebAuthn userVerification=required, sign_count replay detection, rate limit counter in Admin 専用 Valkey, JWT role claim stale 化防止のため認可は DB current role を使用
...
6. docker-compose でデプロイ: `docker compose up -d admin`（compose file は `admin` service、`admin-valkey` service、Admin DB 接続、`OPENSEARCH_URL`、`PRODUCT_OPENSEARCH_INDEX_PREFIX`、`ADMIN_OPENSEARCH_AUDIT_INDEX_PREFIX` を含む）
