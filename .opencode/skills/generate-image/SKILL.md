...
- `codex exec` の安全な起動引数。
- Codex JSONL / `generated_images` artifact の回収、出力 path への配置、存在確認。
...
- Codex には画像生成だけを担当させます。生成 artifact の指定出力 path への copy は `generate-image.mjs` が Node 側で行います。
- sandbox helper が `bwrap` 等で失敗する環境でも、`--json` event または `CODEX_HOME/generated_images` から生成 artifact を回収します。
...
- 出力ファイルが作られない場合は、同じ引数に `--dry-run` を付けて prompt と command を確認します。実生成で Codex artifact が複数見つかった場合は誤コピーを避けるため明示エラーになります。
