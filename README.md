...
- `web` は公開面 LP なので `domain` / `api` に依存しません（`ui` のみ利用可）
...
- Frontend dependency direction: `web -> frontend/ui` (`packages/web` は公開面 LP なので domain/api に依存しない)、`frontend/app -> frontend/domain -> frontend/api` (also `frontend/app -> frontend/ui`)
