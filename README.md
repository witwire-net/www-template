...
| ツール             | pnpm 11.6.0, ESLint 9, Prettier, golangci-lint 1.64, Husky |
| CI                 | GitHub Actions                                             |
...
├── .vscode/                     # VSCode プロジェクト設定（拡張推奨 / formatter / task）
...
VSCode の task から `pnpm dev:*`、`pnpm lint`、`pnpm check`、`pnpm test:run`、`pnpm build` を実行できます。すべて Dev Container 内の `pnpm` と toolchain を使います。
...
- Node.js 24.12+
- pnpm 11.6.0（`corepack enable` で有効化）
- Go 1.26.5+
- PostgreSQL 18
...
| `ALLOWED_ORIGINS`           | `http://www.localhost:5173,http://localhost:5174` | CORS 許可オリジン（カンマ区切り）                |
