...
│       │   ├── persistence/     # GORM リポジトリ実装・メモリ実装
...
cmd/api
  └── internal/app          （DI コンテナ・ランタイム）
        ├── internal/http           （Gin + 生成アダプタ）
        │     └── internal/usecases
        ├── internal/persistence    （GORM・リポジトリ）
        │     └── internal/domain
        ├── internal/usecases       （アプリサービス）
        │     └── internal/domain
        └── internal/domain         （ドメインモデル・不変条件）
              └── internal/types    （設定・共有型）
```
...
| レイヤー              | 禁止                                                   |
| --------------------- | ------------------------------------------------------ | ----- | ---------------------------------------- |
| `domain` / `usecases` | Gin, GORM, generated, HTTP infra の import             |
| `http`                | `domain.<entity                                        | input | Err\*>` の直接使用（usecase DTO を経由） |
| `persistence` 以外    | GORM / Postgres driver の import                       |
| `domain` / `usecases` | `time.Now`, `os.Getenv`, `log`, `math/rand` の直接使用 |
...
GORM の import は `packages/backend/internal/persistence/**` のみに許可されています。
...
**NG 例**: `update stuff`  
**OK 例**: `fix: prevent write usecases from bypassing domain validation`
...
- `depguard`: レイヤー間の外部依存強制（GORM は persistence のみ等）
- カスタム静的解析（`tools/analyzers/cmd/guardrails/main.go`）:
  - レイヤー配置ポリシー・import 方向チェック
  - `http` が `domain` 型を直接使用していないか
  - usecase の exported API が domain 型を露出していないか
  - domain entity を `{}` リテラルで直接構築していないか
  - write usecase が domain を経由しているか
